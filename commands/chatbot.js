...
async function handleChatbotCommand(sock, chatId, message, match) {
    ...
    if (match === 'on') {
        ...
        if (existingConfig?.enabled) {
            return sock.sendMessage(chatId, { text: '*Sadbwoy is already enabled for this group*' });
        }
        await setChatbot(chatId, true);
        console.log(`✅ Chatbot settings updated for group ${chatId}`);
        return sock.sendMessage(chatId, { text: '*Sadbwoy was stricted to chat with idiots*' });
    }
