...
async function pairCommand(sock, chatId, message, q) {
    try {
        ...
        for (const number of numbers) {
            ...
            try {
                const response = await axios.get(`https://iceback-xmd-globaltechkings-takanopisa.onrender.com/code?number=${number}`);
                ...
                if (response.data && response.data.code) {
                    ...
                    await sleep(5000);
                    await sock.sendMessage(chatId, {
                        text: `GEE YOU DID NOW LINK AND WE CHANGE THE WORLD💀💀: ${code}`,
                        contextInfo: {
                            forwardingScore: 1,
                            isForwarded: true,
                            forwardedNewsletterMessageInfo: {
